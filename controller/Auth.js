...
const registerUser = async (req, res)=>{
    ...
    if(emailAlreadyExists) throw new BadRequestError('Email is in use')
    const numberAlreadyExist = await User.findOne({phoneNumber}).lean()
    if(numberAlreadyExist) throw new BadRequestError('Phone number already in use')
...
const loginUser = async (req, res)=>{
    ...
    if(!email || !password) throw new BadRequestError('Please enter email and passowrd')
    const user = await User.findOne({email})
    if(!user) throw new UnauthenticatedError('Incorrect email')
...
const generateOTP = async (req, res)=>{
    ...
    if(otpExist) await OTP.updateOne({email: email}, {otp: otp})
    else await OTP.create({email: email, otp: otp})
    let transporter = nodemailer.createTransport({
        service: 'gmail',
        auth: {
          user: 'yaga77855@gmail.com',
          pass: 'pjtw sowf oyrb qjgn'
        }
    });
    let mailOptions = {
...
const verifyOTP = async(req, res)=>{
 ...
 if(!checkOTP) throw new UnauthenticatedError('Invalid OTP')
 if(Number(otp) != checkOTP.otp) throw new UnauthenticatedError('Incorrect OTP')
 res.sendStatus(200)
}
const resetPassword = async(req, res)=>{
    ...
    await User.updateOne({email: email}, {password: password}).lean()
    res.sendStatus(202)
}
